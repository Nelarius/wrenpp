...
struct Vec3
{
    ...
    Vec3 plus(const Vec3& rhs) const { return Vec3{x + rhs.x, y + rhs.y, z + rhs.z}; }
};

struct Transform
{
    Transform(const Vec3& p) : position(p) {}
...
void printConstRefString(const std::string& str) { std::printf("%s\n", str.c_str()); }

void printValueString(std::string str) { std::printf("%s\n", str.c_str()); }

void printCharString(const char* str) { std::printf("%s\n", str); }
